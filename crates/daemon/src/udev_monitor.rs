        ...
        match guard.try_io(|inner| {
            ...
            match sock.receive_event() {
                Ok(event) => {
                    ...
                    let devnode = event.devnode().and_then(|p| p.to_str()).unwrap_or("").to_string();
                    let vendor = event.property_value("ID_VENDOR_ID").and_then(|s| s.to_str()).map(|s| format!("0x{}", s)).unwrap_or_default();
                    let product = event.property_value("ID_MODEL_ID").and_then(|s| s.to_str()).map(|s| format!("0x{}", s)).unwrap_or_default();
                    let serial = event.property_value("ID_SERIAL_SHORT").and_then(|s| s.to_str()).unwrap_or("").to_string();
                    let dtype = event.property_value("ID_USB_DRIVER").and_then(|s| s.to_str()).unwrap_or("").to_string();
                    ...
                    tokio::spawn(async move {
                        if let Ok(ctx) = zbus::SignalContext::new(&conn, DBUS_PATH) {
                            if action == "add" {
                                let _ = crate::dbus::DaemonState::unknown_device_inserted(&ctx, &info).await;
                            } else if action == "remove" {
                                let _ = crate::dbus::DaemonState::device_removed(&ctx, &info.id).await;
                            }
                        }
                    });
                    Ok(())
